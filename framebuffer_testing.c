...
// Ta funkcija poskrbi, da se:
// - odpre datoteka, ki predstavlja
void get_screeninfo(int *fbfd, struct fb_var_screeninfo *vinfo,
                    struct fb_fix_screeninfo *finfo) {
    // Odpre datoteko, ki predstavlja virtualni MO v načinu read/write
...
int main() {
    ...
    // Figure out the size of the screen in bytes (that's why we divide by 8).
    screensize = vinfo.xres * vinfo.yres * vinfo.bits_per_pixel / 8;

    // Map the device to memory
    fbp =
        (unsigned char *) mmap(0, screensize, PROT_READ | PROT_WRITE, MAP_SHARED, fbfd, 0);
